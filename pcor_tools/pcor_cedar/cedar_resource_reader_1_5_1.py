...
class CedarResourceReader_1_5_1(CedarResourceReader):
    ...
    def extract_program_data(contents_json, key='PROGRAM'):
        ...
        program.name = PcorTemplateParser.sanitize_column(contents_json[key]["Program_name"]["@value"])
    ...
    def extract_resource_data(contents_json, key='RESOURCE'):
        ...
        resource.id = PcorTemplateParser.sanitize_column(contents_json[key]["resource_GUID"]["@value"])
        resource.resource_type = PcorTemplateParser.sanitize_column(contents_json[key]["resource_type"]["@value"])
        resource.name = PcorTemplateParser.sanitize_column(contents_json[key]["resource_name"]["@value"])
        resource.short_name = PcorTemplateParser.sanitize_column(contents_json[key]["resource_short_name"]["@value"])

        resource.resource_url = PcorTemplateParser.sanitize_column(contents_json[key]["resource_url"]["@id"])
        resource.description = PcorTemplateParser.sanitize_column(contents_json[key]["resource_description"]["@value"])

        for domain in contents_json[key]["domain"]:
            if domain["@value"]:
                resource.domain.append(PcorTemplateParser.sanitize_column(domain["@value"]))
            if resource.domain:
        ...
        for domain in contents_json[key]["domain_other"]:
            if domain["@value"]:
                resource.domain_other.append(PcorTemplateParser.sanitize_column(domain["@value"]))

        for item in contents_json[key]["access_type"]:
            if item["@value"]:
                resource.access_type.append(PcorTemplateParser.sanitize_column(item["@value"]))

        # FixMe: need to convert string to DateTime format
        resource.created_datetime = PcorTemplateParser.sanitize_column(contents_json["pav:createdOn"])
        resource.updated_datetime = PcorTemplateParser.sanitize_column(contents_json["pav:lastUpdatedOn"])

        resource.verification_datetime = PcorTemplateParser.sanitize_column(contents_json[key]["date_verified"]["@value"])
        resource.verification_datetime = PcorTemplateParser.sanitize_column(contents_json[key]["date_verified"]["@value"])

        for publication_citation in contents_json[key]["Publication"]["publication_citation"]:
            resource.publications.append(PcorTemplateParser.sanitize_column(publication_citation["@value"]))

        for publication_link in contents_json[key]["Publication"]["publication_link"]:
            if '@id' in publication_link and publication_link["@id"]:
                resource.publication_links.append(PcorTemplateParser.sanitize_column(publication_link["@id"]))

        for keyword in contents_json[key]["keywords"]:
            if keyword["@value"] != 'Climate Change' and keyword["@value"] != 'Climate Changes':
                resource.keywords.append(PcorTemplateParser.sanitize_column(keyword["@value"]))
        ...
        if contents_json[key]["Resource Reference_150"]["resource_reference_link"]:
            resource.resource_reference_link = PcorTemplateParser.sanitize_column(contents_json[key]["Resource Reference_150"]["resource_reference_link"]["@id"])

        resource.resource_use_agreement = PcorTemplateParser.sanitize_column(contents_json[key]["Resource Use Agreement_150"]["resource_use_agreement"]["@value"])

        # pop data can have an empty {}} with no @id:null
        resource.resource_use_agreement_link = PcorTemplateParser.sanitize_column(contents_json[key]["Resource Use Agreement_150"]["resource_use_agreement_link"].get("@id"))

        resource.is_static = PcorTemplateParser.sanitize_boolean(contents_json[key]["is_static"]["@value"])

        resource.resource_version = resource.verification_datetime = PcorTemplateParser.sanitize_column(contents_json[key]["resource_version"]["@value"])
    ...
    def extract_geoexposure_data(contents_json, data_resource_key="DATA RESOURCE", key="GEOEXPOSURE DATA"):
        ...
        geoexposure.comments = PcorTemplateParser.sanitize_column(contents_json[data_resource_key]["Comments"]["@value"])
        geoexposure.intended_use = PcorTemplateParser.sanitize_column(contents_json[data_resource_key]["intended_use"]["@value"])

        for item in contents_json[data_resource_key]["source_name"]:
            if item["@value"]:
                geoexposure.source_name.append(PcorTemplateParser.sanitize_column(item["@value"]))
        ...
        for update_frequency in contents_json[data_resource_key]["update_frequency"]:
            if update_frequency["@value"]:
                geoexposure.update_frequency.append(PcorTemplateParser.sanitize_column(update_frequency["@value"]))

        geoexposure.update_frequency_other = PcorTemplateParser.sanitize_column(contents_json[data_resource_key]["update_frequency_other"]["@value"])
        ...
        for measure in contents_json[key]["measures"]:
            if measure["@value"]:
                geoexposure.measures.append(PcorTemplateParser.sanitize_column(measure["@value"]))

        for measure in contents_json[key]["measures_other"]:
            if measure["@value"]:
                geoexposure.measures_other.append(PcorTemplateParser.sanitize_column(measure["@value"]))

        for measurement_method in contents_json[key]["measurement_method"]:
            if measurement_method["@value"]:
                geoexposure.measurement_method.append(PcorTemplateParser.sanitize_column(measurement_method["@value"]))

        for measurement_method in contents_json[key]["measurement_method_other"]:
            if measurement_method["@value"]:
                geoexposure.measurement_method_other.append(PcorTemplateParser.sanitize_column(measurement_method["@value"]))

        geoexposure.time_extent_start_yyyy = (
            PcorTemplateParser.format_date_time(PcorTemplateParser.sanitize_column(contents_json[key]["time_extent_start"]["@value"])))
        geoexposure.time_extent_end_yyyy = (
            PcorTemplateParser.format_date_time(PcorTemplateParser.sanitize_column(contents_json[key]["time_extent_end"]["@value"])))

        geoexposure.time_available_comment = PcorTemplateParser.sanitize_column(contents_json[key]["time_available_comment"]["@value"])

        for temporal_resolution in contents_json[key]["temporal_resolution"]:
            if temporal_resolution["@value"]:
                geoexposure.temporal_resolution.append(PcorTemplateParser.sanitize_column(temporal_resolution["@value"]))

        for temporal_resolution in contents_json[key]["temporal_resolution_other"]:
            if temporal_resolution["@value"]:
                geoexposure.temporal_resolution_other.append(PcorTemplateParser.sanitize_column(temporal_resolution["@value"]))

        for spatial_resolution in contents_json[key]["spatial_resolution"]:
            if spatial_resolution["@value"]:
                geoexposure.spatial_resolution.append(PcorTemplateParser.sanitize_column(spatial_resolution["@value"]))

        for spatial_resolution in contents_json[key]["spatial_resolution_other"]:
            if spatial_resolution["@value"]:
                geoexposure.spatial_resolution_other.append(PcorTemplateParser.sanitize_column(spatial_resolution["@value"]))

        for spatial_coverage in contents_json[key]["spatial_coverage"]:
            if spatial_coverage["@value"]:
                geoexposure.spatial_coverage.append(PcorTemplateParser.sanitize_column(spatial_coverage["@value"]))

        for spatial_coverage in contents_json[key]["spatial_coverage_other"]:
            if spatial_coverage["@value"]:
                geoexposure.spatial_coverage_other.append(PcorTemplateParser.sanitize_column(spatial_coverage["@value"]))

        for box in contents_json[key]["spatial_bounding_box"]:
            if box["@value"]:
                geoexposure.spatial_bounding_box.append(PcorTemplateParser.sanitize_column(box["@value"]))

        for source in contents_json[key]["geometry_source"]:
            if source["@value"]:
                geoexposure.geometry_source.append(PcorTemplateParser.sanitize_column(source["@value"]))

        for source in contents_json[key]["geometry_source_other"]:
            if source["@value"]:
                geoexposure.geometry_source_other.append(PcorTemplateParser.sanitize_column(source["@value"]))

        for model_method in contents_json[key]["model_methods"]:
            if model_method["@value"]:
                geoexposure.model_methods.append(PcorTemplateParser.sanitize_column(model_method["@value"]))

        for geometry_type in contents_json[key]["geometry_type"]:
            if geometry_type["@value"]:
                geoexposure.geometry_type.append(PcorTemplateParser.sanitize_column(geometry_type["@value"]))

        for exposure_media in contents_json[key]["exposure_media"]:
            if exposure_media["@value"]:
                geoexposure.exposure_media.append(PcorTemplateParser.sanitize_column(exposure_media["@value"]))

        for geographic_feature in contents_json[key]["geographic_feature"]:
            if geographic_feature["@value"]:
                geoexposure.geographic_feature.append(PcorTemplateParser.sanitize_column(geographic_feature["@value"]))

        for feature in contents_json[key]["geographic_feature_other"]:
            if feature["@value"]:
                geoexposure.geographic_feature_other.append(PcorTemplateParser.sanitize_column(feature["@value"]))

        for data_format in contents_json[key]["data_formats"]:
            if data_format["@value"]:
                geoexposure.data_formats.append(PcorTemplateParser.sanitize_column(data_format["@value"]))

        for item in contents_json[key]["Data Download"]["data_location_text"]:
            if item["@value"]:
                geoexposure.data_location_text.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in contents_json[key]["Data Download"]["data_link"]:
            if '@id' in item and item["@id"]:
                geoexposure.data_link.append(PcorTemplateParser.sanitize_column(item["@id"]))
    ...
    def extract_geoexposure_tool_data(contents_json,data_resource_key="DATA_RESOURCE", key="GEOEXPOSURE DATA"):
        ...
        for tool_type in body["tool_type"]:
            if tool_type["@value"]:
                geotool.tool_type.append(PcorTemplateParser.sanitize_column(tool_type["@value"]))

        for tool_type in body["tool_type_other"]:
            if tool_type["@value"]:
                geotool.tool_type_other.append(PcorTemplateParser.sanitize_column(tool_type["@value"]))

        for os in body["operating_system"]:
            if os["@value"]:
                geotool.operating_system.append(PcorTemplateParser.sanitize_column(os["@value"]))

        for os in body["operating_system_other"]:
            if os["@value"]:
                geotool.operating_system_other.append(PcorTemplateParser.sanitize_column(os["@value"]))

        for lang in body["languages"]:
            if lang["@value"]:
                geotool.languages.append(PcorTemplateParser.sanitize_column(lang["@value"]))

        for lang in body["languages_other"]:
            if lang["@value"]:
                geotool.languages_other.append(PcorTemplateParser.sanitize_column(lang["@value"]))

        for license_type in body["license_type"]:
            if license_type["@value"]:
                geotool.license_type.append(PcorTemplateParser.sanitize_column(license_type["@value"]))

        for license_type in body["license_type_other"]:
            if license_type["@value"]:
                geotool.license_type_other.append(PcorTemplateParser.sanitize_column(license_type["@value"]))

        for aud in body["suggested_audience"]:
            if aud["@value"]:
                geotool.suggested_audience.append(PcorTemplateParser.sanitize_column(aud["@value"]))

        geotool.is_open = PcorTemplateParser.sanitize_boolean(
            body["is_open"]["@value"])

        geotool.intended_use = PcorTemplateParser.sanitize_column(body["intended_use"]["@value"])
    ...
    def extract_population_data(contents_json, data_resource_key="DATA_RESOURCE", key="POPULATION DATA RESORCE"):
        ...
        for item in data_resource["source_name"]:
            if item["@value"]:
                population.source_name.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in data_resource["update_frequency"]:
            if item["@value"]:
                population.update_frequency.append(PcorTemplateParser.sanitize_column(item["@value"]))
        population.update_frequency_other = PcorTemplateParser.sanitize_column(data_resource["update_frequency_other"]["@value"])
        ...
        population.comments = PcorTemplateParser.sanitize_column(data_resource["Comments"]["@value"])
        population.intended_use = PcorTemplateParser.sanitize_column(data_resource["intended_use"]["@value"])
        ...
        for item in pop_data_json["exposure_media"]:
            if item["@value"]:
                population.exposure_media.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in pop_data_json["measures"]:
            if item["@value"]:
                population.measures.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in pop_data_json["measures_others"]:
            if item["@value"]:
                population.measures_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        population.time_extent_start_yyyy = (
            PcorTemplateParser.format_date_time(PcorTemplateParser.sanitize_column(pop_data_json["time_extent_start"]["@value"])))
        population.time_extent_end_yyyy = (
            PcorTemplateParser.format_date_time(PcorTemplateParser.sanitize_column(pop_data_json["time_extent_end"]["@value"])))
        population.time_available_comment = PcorTemplateParser.sanitize_column(pop_data_json["time_available_comment"]["@value"])

        if type(pop_data_json["temporal_resolution"]) in (tuple, list):
            for item in pop_data_json["temporal_resolution"]:
                if item["@value"]:
                    population.temporal_resolution.append(PcorTemplateParser.sanitize_column(item["@value"]))
        elif pop_data_json["temporal_resolution"]["@value"]:
            population.temporal_resolution.append(PcorTemplateParser.sanitize_column(pop_data_json["temporal_resolution"]["@value"]))

        for item in pop_data_json["temporal_resolution_other"]:
            if item["@value"]:
                population.temporal_resolution_other.append(PcorTemplateParser.sanitize_column(item["@value"]))

        if type(pop_data_json["spatial_resolution"]) in (tuple, list):
            for item in pop_data_json["spatial_resolution"]:
                if item["@value"]:
                    population.spatial_resolution.append(PcorTemplateParser.sanitize_column(item["@value"]))
                else:
                    population.spatial_resolution.append(PcorTemplateParser.sanitize_column(pop_data_json["spatial_resolution"]["@value"]))

        for item in pop_data_json["spatial_resolution_other"]:
            if item["@value"]:
                population.spatial_resolution_other.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in pop_data_json["spatial_coverage"]:
            if item["@value"]:
                population.spatial_coverage.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for spatial_coverage in pop_data_json["spatial_coverage_other"]:
            if spatial_coverage["@value"]:
                population.spatial_coverage_other.append(PcorTemplateParser.sanitize_column(spatial_coverage["@value"]))

        for item in pop_data_json["geometry_type"]:
            if item["@value"]:
                population.geometry_type.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in pop_data_json["geometry_source"]:
            if item["@value"]:
                population.geometry_source.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in pop_data_json["geometry_source_other"]:
            if item["@value"]:
                population.geometry_source_other.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in pop_data_json["model_methods"]:
            if item["@value"]:
                population.model_methods.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in pop_data_json["model_methods_other"]:
            if item["@value"]:
                population.model_methods_other.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for population_study in pop_data_json["population_studied"]:
            if population_study["@value"]:
                population.population_studied.append(PcorTemplateParser.sanitize_column(population_study["@value"]))

        for item in pop_data_json["population_studied_other"]:
            if item["@value"]:
                population.population_studied_other.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in pop_data_json["biospecimens_type"]:
            if item["@value"]:
                population.biospecimens_type.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in pop_data_json["data_formats"]:
            if item["@value"]:
                population.data_formats.append(PcorTemplateParser.sanitize_column(item["@value"]))
        ...
        for item in pop_data_json["Data Download"]["data_location_text"]:
            if item["@value"]:
                population.data_location_text.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in pop_data_json["Data Download"]["data_link"]:
            if '@id' in item and item["@id"]:
                population.data_link.append(PcorTemplateParser.sanitize_column(item["@id"]))

        population.linkable_encounters = PcorTemplateParser.sanitize_boolean(
            PcorTemplateParser.sanitize_column(pop_data_json["linkable_encounters"]["@value"]))

        for item in pop_data_json["biospecimens_type"]:
            if item["@value"]:
                PcorTemplateParser.sanitize_column(population.biospecimens_type.append(item["@value"]))
    ...
    def extract_key_dataset_data(contents_json, data_resource_key="DATA_RESOURCE", key="KEY DATASETS DATA"):
        ...
        for item in data_resource["source_name"]:
            if item["@value"]:
                key_dataset.source_name.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in data_resource["update_frequency"]:
            if item["@value"]:
                key_dataset.update_frequency.append(PcorTemplateParser.sanitize_column(item["@value"]))
        key_dataset.update_frequency_other = PcorTemplateParser.sanitize_column(data_resource["update_frequency_other"]["@value"])
        ...
        for item in key_data_json["measures"]:
            if item["@value"]:
                PcorTemplateParser.sanitize_column(key_dataset.measures.append(item["@value"]))
        for item in key_data_json["Measures_other"]:
            if item["@value"]:
                key_dataset.measures_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["measurement_method"]:
            if item["@value"]:
                key_dataset.measurement_method.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["measurement_method_other"]:
            if item["@value"]:
                key_dataset.measurement_method_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        key_dataset.time_extent_start_yyyy = (
            PcorTemplateParser.format_date_time(PcorTemplateParser.sanitize_column(key_data_json["time_extent_start"]["@value"])))
        key_dataset.time_extent_end_yyyy = (
            PcorTemplateParser.format_date_time(PcorTemplateParser.sanitize_column(key_data_json["time_extent_end"]["@value"])))
        key_dataset.time_available_comment = PcorTemplateParser.sanitize_column(key_data_json["time_available_comment"]["@value"])
        if key_data_json["temporal_resolution"]["@value"]:
                key_dataset.temporal_resolution.append(PcorTemplateParser.sanitize_column(key_data_json["temporal_resolution"]["@value"]))
        for item in key_data_json["temporal_resolution_other"]:
            if item["@value"]:
                key_dataset.temporal_resolution_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["temporal_resolution_all_available"]:
            if item["@value"]:
                key_dataset.temporal_resolution_all_available.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["temporal_resolution_all_other_available"]:
            if item["@value"]:
                key_dataset.temporal_resolution_all_other_available.append(PcorTemplateParser.sanitize_column(item["@value"]))
        key_dataset.temporal_resolution_comment = PcorTemplateParser.sanitize_column(key_data_json["temporal_resolution_comment"]["@value"])
        if key_data_json["spatial_resolution"]["@value"]:
                key_dataset.spatial_resolution.append(PcorTemplateParser.sanitize_column(key_data_json["spatial_resolution"]["@value"]))
        for item in key_data_json["spatial_resolution_other"]:
            if item["@value"]:
                key_dataset.spatial_resolution_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["spatial_resolution_all_available"]:
            if item["@value"]:
                key_dataset.spatial_resolution_all_available.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["spatial_resolution_all_other_available"]:
            if item["@value"]:
                key_dataset.spatial_resolution_all_other_available.append(PcorTemplateParser.sanitize_column(item["@value"]))
        key_dataset.spatial_resolution_comment = PcorTemplateParser.sanitize_column(key_data_json["spatial_resolution_comment"]["@value"])
        for item in key_data_json["spatial_coverage"]:
            if item["@value"]:
                key_dataset.spatial_coverage.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["spatial_coverage_other"]:
            if item["@value"]:
                key_dataset.spatial_coverage_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["spatial_bounding_box"]:
            if item["@value"]:
                key_dataset.spatial_bounding_box.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["geometry_type"]:
            if item["@value"]:
                key_dataset.geometry_type.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["geometry_source"]:
            if item["@value"]:
                key_dataset.geometry_source.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["geometry_source_other"]:
            if item["@value"]:
                key_dataset.geometry_source_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["model_methods"]:
            if item["@value"]:
                key_dataset.model_methods.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["model_methods_other"]:
            if item["@value"]:
                key_dataset.model_methods_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["exposure_media"]:
            if item["@value"]:
                key_dataset.exposure_media.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["geographic_feature"]:
            if item["@value"]:
                key_dataset.geographic_feature.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["geographic_feature_other"]:
            if item["@value"]:
                key_dataset.geographic_feature_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["data_formats"]:
            if item["@value"]:
                key_dataset.data_formats.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["Data Download"]["data_location_text"]:
            if item["@value"]:
                key_dataset.data_location_text.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["Data Download"]["data_link"]:
            if '@id' in item and item["@id"]:
                key_dataset.data_link.append(PcorTemplateParser.sanitize_column(item["@id"]))
        for item in key_data_json["license_type"]:
            if item["@value"]:
                key_dataset.license_type.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["license_type_other"]:
            if item["@value"]:
                key_dataset.license_type_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["use_suggested"]:
            if item["@value"]:
                key_dataset.use_suggested.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["use_suggested_other"]:
            if item["@value"]:
                key_dataset.use_suggested_other.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["use_limitations"]:
            if item["@value"]:
                key_dataset.use_limitations.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in key_data_json["use_key_variables"]:
            if item["@value"]:
                key_dataset.use_key_variables.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in key_data_json["suggested_audience"]:
            if item["@value"]:
                key_dataset.suggested_audience.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in key_data_json["Dataset Tools"]["use_tool_link"]:
            if item:
                key_dataset.use_tool_link.append(PcorTemplateParser.sanitize_column(item["@id"]))
            else:
                key_dataset.use_tool_link.append('')

        for item in key_data_json["Dataset Tools"]["use_tools_text"]:
            if item["@value"]:
                key_dataset.use_tools_text.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in key_data_json["Example Application"]["use_example_application_link"]:
            if item["@value"]: # FIXME: link uses value not id - mcc
                key_dataset.use_example_application_link.append(PcorTemplateParser.sanitize_column(item["@value"]))
        for item in key_data_json["Example Application"]["Use_example_application_text"]:
            if item["@value"]:
                key_dataset.use_example_application_text.append(PcorTemplateParser.sanitize_column(item["@value"]))

        for item in key_data_json["use_strengths"]:
            if item["@value"]:
                key_dataset.use_strengths.append(PcorTemplateParser.sanitize_column(item["@value"]))
        ...
        for item in key_data_json["use_example_metrics"]:
            if item["@value"]:
                key_dataset.use_example_metrics.append(PcorTemplateParser.sanitize_column(item["@value"]))
